...
subplot(2,2,1); 
    plot(act_level, slope_b*180/pi, '-', Marker='diamond', LineWidth=3.5, Color="#0072BD"); hold on; 
 %   plot(act_level1, slope_1*180/pi, 'ro-');
 %   plot(act_level2, slope_2*180/pi, 'go-');
    plot(act_level3, slope_3*180/pi, 'x-', LineWidth=2, Color="#D95319"); hold on;
    plot(act_level4, slope_4*180/pi, '*-', LineWidth=2, Color="#EDB120");
    plot(act_level5, slope_5*180/pi, 'o-', LineWidth=2, Color="#7E2F8E");
    box on; grid on; title("Slopes"); xlabel("Actual Level [adim]"); ylabel("[deg]"); legend(["Base","EET3", "ET2","ET3"],"Location","southeast"); hold off;
    set(gca, 'FontSize', 16);
subplot(2,2,2); 
    plot(act_level, step_height_b*100, '-', Marker='diamond', LineWidth=3.5, Color="#0072BD"); hold on; 
 %   plot(act_level1, step_height_1*100, 'ro-');
  %  plot(act_level2, step_height_2*100, 'go-');
    plot(act_level3, step_height_3*100, 'x-', LineWidth=2, Color="#D95319"); hold on;
    plot(act_level4, step_height_4*100, '*-', LineWidth=2, Color="#EDB120");
    plot(act_level5, step_height_5*100, 'o-', LineWidth=2, Color="#7E2F8E");
    box on; grid on; title("Steps height"); xlabel("Actual Level [adim]"); ylabel("[cm]"); legend(["Base","EET3", "ET2","ET3"],"Location","northwest"); hold off;
    set(gca, 'FontSize', 16);
subplot(2,2,3); 
    plot(act_level, discrete_obstacles_height_b*100, '-', Marker='diamond', LineWidth=3.5, Color="#0072BD"); hold on;
%    plot(act_level1, discrete_obstacles_height_1*100, 'ro-');
%    plot(act_level2, discrete_obstacles_height_2*100, 'go-');
    plot(act_level3, discrete_obstacles_height_3*100, 'x-', LineWidth=2, Color="#D95319"); hold on;
    plot(act_level4, discrete_obstacles_height_4*100, '*-', LineWidth=2, Color="#EDB120");
    plot(act_level5, discrete_obstacles_height_5*100, 'o-', LineWidth=2, Color="#7E2F8E");
    box on; grid on; title("Discrete obstacles height"); xlabel("Actual Level [adim]"); ylabel("[cm]"); legend(["Base","EET3", "ET2","ET3"],"Location","northwest"); hold off;
    set(gca, 'FontSize', 16);
subplot(2,2,4); 
    plot(act_level, stepping_stones_size_b, '-', Marker='diamond', LineWidth=3.5, Color="#0072BD"); hold on;
%    plot(act_level1, stepping_stones_size_1, 'ro-');
  %  plot(act_level2, stepping_stones_size_2, 'go-');
    plot(act_level3, stepping_stones_size_3, 'x-', LineWidth=2, Color="#D95319"); hold on;
    plot(act_level4, stepping_stones_size_4, '*-', LineWidth=2, Color="#EDB120");
    plot(act_level5, stepping_stones_size_5, 'o-', LineWidth=2, Color="#7E2F8E");
    box on; grid on; title("Larghezza pedane"); xlabel("livello attuale [adim]"); ylabel("stepping stones size [m]"); legend(["Base","EET3",  "ET2","ET3"]); hold off;
    set(gca, 'FontSize', 16);
subplot(2, 2, 4);
    plot(act_level3, wave_amplitude_3*100, 'x-', LineWidth=2, Color="#D95319"); hold on;
    plot(act_level4, wave_amplitude_4*100, '*-', LineWidth=2, Color="#EDB120");
    plot(act_level5, wave_amplitude_5*100, 'o-', LineWidth=2, Color="#7E2F8E");
